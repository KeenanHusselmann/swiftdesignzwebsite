/**
 * record-logo-showcase.mjs — Swift Designz Logo Showcase Post
 * ─────────────────────────────────────────────────────────────
 * Records video-post-3-logo-showcase.html using CDP screencast.
 * CSS-only animations — no canvas, screencast works reliably.
 *
 * Output: public/video-output/logo-showcase.mp4  (1080×1920 · H.264 · 30fps)
 *
 * Usage: node scripts/record-logo-showcase.mjs
 */
...
const HTML       = path.join(ROOT, 'public', 'video-post-3-logo-showcase.html');
const OUT_DIR    = path.join(ROOT, 'public', 'video-output');
const FRAMES_DIR = path.join(OUT_DIR, '_frames_logo_tmp');
const MP4_OUT    = path.join(OUT_DIR, 'logo-showcase.mp4');
const CHROME     = 'C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe';

const VIEWPORT    = { width: 1080, height: 1920, deviceScaleFactor: 1 };
const RECORD_SECS = 15;   // covers all intro animations + full loop
const OUTPUT_FPS  = 30;
...
(async () => {
  console.log('\n  Swift Designz — Logo Showcase Recorder');
  console.log('  ────────────────────────────────────────\n');

  if (!fs.existsSync(HTML)) { console.error(`  ERROR: ${HTML} not found.`); process.exit(1); }
  ...
  for (let i = 0; i < kept.length; i++) {
    ...
    lines.push(`duration ${i < kept.length - 1 ? (kept[i + 1].ts - kept[i].ts).toFixed(6) : '0.033'}`);
  }
  const concatFile = path.join(FRAMES_DIR, 'frames.txt');
  fs.writeFileSync(concatFile, lines.join('\n'));

  // ── Encode 1080×1920 H.264 ──────────────────────────────────────────────
  ...
  console.log(`  Done! ${mb(MP4_OUT)} MB`);
  console.log(`  Output : public/video-output/logo-showcase.mp4`);
