/**
 * record-client-milestone-post.mjs — Swift Designz April 2026
 * ─────────────────────────────────────────────────────────────────────────────
 * Records video-post-4-client-milestone.html as a mobile HD MP4.
 * CSS-only animations → CDP screencast works reliably here.
 *
 * Output: public/video-output/client-milestone-post.mp4
 * Format: 1080×1920 · H.264 · CRF 15 · 30fps · mobile portrait
 *
 * Usage: node scripts/record-client-milestone-post.mjs
 */
...
const HTML      = path.join(ROOT, 'public', 'video-post-4-client-milestone.html');
const OUT_DIR   = path.join(ROOT, 'public', 'video-output');
const FRAMES_DIR = path.join(OUT_DIR, '_frames_milestone_tmp');
const MP4_OUT   = path.join(OUT_DIR, 'client-milestone-post.mp4');
...
(async () => {
  console.log('\n  Swift Designz — Client Milestone Post Recorder');
  console.log('  ────────────────────────────────────────────────');
  ...
  console.log(`\n  Done! ${mb(MP4_OUT)} MB`);
  console.log(`  Output: public/video-output/client-milestone-post.mp4`);
  console.log(`  Format: 1080×1920 · H.264 · ${OUTPUT_FPS}fps · mobile portrait\n`);
})();
