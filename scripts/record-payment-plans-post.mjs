/**
 * record-payment-plans-post.mjs — Swift Designz April 2026
 * ─────────────────────────────────────────────────────────────────────────────
 * Records video-post-5-payment-plans.html as a mobile HD MP4.
 * CSS-only animations → CDP screencast works reliably here.
 *
 * Output: public/video-output/payment-plans-post.mp4
 * Format: 1080×1920 · H.264 · CRF 15 · 30fps · mobile portrait
 *
 * Usage: node scripts/record-payment-plans-post.mjs
 */
...
const HTML      = path.join(ROOT, 'public', 'video-post-5-payment-plans.html');
const OUT_DIR   = path.join(ROOT, 'public', 'video-output');
const FRAMES_DIR = path.join(OUT_DIR, '_frames_payment_tmp');
const MP4_OUT   = path.join(OUT_DIR, 'payment-plans-post.mp4');
...
(async () => {
  console.log('\n  Swift Designz — Payment Plans Post Recorder');
  console.log('  ─────────────────────────────────────────────');
